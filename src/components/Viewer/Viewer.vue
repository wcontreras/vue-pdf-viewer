<template>
  <CompIFrame :css="$options.viewerStyle" ref="iframe">
    <div class="viewer-container" ref="container">
      <div class="scroller viewer" ref="viewerScroller" @scroll="handleViewerScroll">
...
export default {
  ...
  computed: {
    ...
    viewerItemStyle() {
      return this.isHorizontalViewer
        ? {
            height: `${(this.zoom / NORMAL_RATIO / 100) * this.viewportWidth}px`,
          }
        : {};
    },
    thumbnailItemStyle() {
  ...
  methods: {
    ...
    handleViewerScroll(evt) {
      ...
      const perHeight =
        (this.viewerContentHeight + MARGIN_OFFSET - this.verticalPadding) / this.total;
